...
class DownloadManager:
    ...
    def connect_to_db(self):
        ...
        db = psycopg2.connect(
            dbname=SECRETS["reddit_images"]["name"],
            user=SECRETS["reddit_images"]["user"],
            # password=SECRETS["reddit_images"]["password"],
            host=SECRETS["reddit_images"]["host"],
        )
    ...
    def insert_media(self, values):
        ...
        if values["type"] == "video":
            media_id = self.get_id("select %s from media", ("max(id)",))
            self.execute_sql(
    ...
    def remove_media(self, file_name):
        ...
        file = self.get_file(sql, file_name)  # ["filename", "ext"]
        if not file:
    ...
    def import_all_yaml_dumps(self, root_dir):
        for directory, subdir, files in os.walk(root_dir):
            for file in files:
                dm.update_db(os.path.join(directory, file))


if __name__ == "__main__":
    dm = DownloadManager(config="s.yaml")
    dm.get_posts()
